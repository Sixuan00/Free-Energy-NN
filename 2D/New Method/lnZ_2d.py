...
class TensorSampling:
    ...
    def psi_func(self, psi, samples):
        psi = psi.view(-1)
        samples = ((samples.detach().cpu().numpy() + 1) / 2).astype(int)
        N = samples.shape[1]
        weights = np.array([2 ** (N - i - 1) for i in range(N)])
        indexes = (samples * weights).sum(axis=1)
        # print(indexes)
        # indexes = torch.from_numpy(index).to(dtype=self.type, device=self.device)
        return psi[indexes]
...
class HonNN:
    ...
    def B_on_middle(self, psi_func, s, beta=None):
        ...
        for i in ilist:
            for j in ilist:
                s0 = torch.cat([I * i, s, I * j], dim=1)
                phi += torch.exp(beta * i * j) * psi_func(s0)
    ...
    def calc_norm(self, H, psi_func, n):
        ...
        s = self.index2sample(r, n)
        s_ = self.index2sample(c, n)
        print(s.shape)
        print(s_.shape)
