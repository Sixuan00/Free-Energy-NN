...
def learning(model, model0, optimizer, loss_func, num_epoch, batch_size, my_device, beta=1.0, memo='init', save_path='./data/', anneal=False):
    ...
    print(model.parameters(), model0.parameters())
    t0 = time.time()
    beta0 = beta
...
def eat_state_(num_site, net_depth, beta, batch_size, num_epoch, l_r, model0, my_type, my_device,save_path, memo='B_'):
    ...
    for site_index in range(num_site):
        ...
        H = lambda beta : calc_H(num_site, site_index, mode='B', beta=beta, my_type=my_type, my_device=my_device)
...
def main():
    ...
    for num_site in [4]:
        for net_depth in [3]:
            print('num_site= {}, net_depth= {}'.format(num_site, net_depth))
            ...
            # print(model0.layers[0].weight.shape)
            log_norm0 = torch.load(model_saving_path + 'init' + '_log_norm_' + '.pth')

            model_saving_path = './model/' + 'test_trial6/' + 'n%d_d%d_b%d_lr%f/' % (num_site, net_depth, batch_size, l_r)
            data_saving_path = './data/' + 'test_trial6/' + 'n%d_d%d_b%d_lr%f/' % (num_site, net_depth, batch_size, l_r)
